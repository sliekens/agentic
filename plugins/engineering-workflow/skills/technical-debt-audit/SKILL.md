...
- **Inverted or borrowed lifecycle** — a component's behaviour depends on state owned or timed by a different component (detection: comments explaining "this fires too late", tokens passed as proxies for state, ObjectDisposedException guards). _Usually points to:_ give the component ownership of its own state, or inject the state/signal explicitly instead of leaning on another component's timing.
...
- **Inlined pipelines** — a sequence of distinct processing steps that are serialised in one function, making it hard to add, remove, or retry a single step. _Usually points to:_ model each step as a discrete stage (handler/middleware) so steps can be added, removed, or retried independently.
...
- **Contract divergence / hidden behaviour** — an implementation does more or less than its signature and documentation promise, so callers cannot predict it from the contract (detection: arguments silently rewritten, e.g. an empty value swapped for a default; filtering, fan-out, or side effects the interface never discloses; behaviour that contradicts the doc comment). _Usually points to:_ make behaviour match the contract — either surface the hidden step in the signature and docs, or remove it and let the caller decide.
