...
Look for problems at the level of concerns and responsibilities, not style or implementation details. Good candidates:

- **God classes / monoliths** — a single class handles multiple distinct concerns (detection: many unrelated injected dependencies, long files, unrelated method groups)
- **Inverted or borrowed lifecycle** — a component's behaviour depends on state owned or timed by a different component (detection: comments explaining "this fires too late", tokens passed as proxies for state, ObjectDisposedException guards)
- **Polling where events would do** — a loop reads a file or checks a flag on a timer when a channel, callback, or event would eliminate the latency and fragility
- **Inlined pipelines** — a sequence of distinct processing steps that are serialised in one function, making it hard to add, remove, or retry a single step
- **N round-trips where one would do** — multiple sequential queries that each fetch a partial view of the same data, especially inside a timed loop
- **Abstraction mismatch** — interfaces introduced at internal seams where only one implementation exists and no test requires stubbing, or conversely, concrete coupling across a boundary that should be swappable
...
Prioritise by impact: lead with the issues that cause real pain (production errors, hard-to-add features, frequent regressions) before cosmetic concerns.
...
After writing, summarise what was documented in a short message: how many issues, which area they're in, and whether any cross-reference each other in a meaningful dependency order. If the user wants to track priority or effort, that's a separate conversation — don't add it to the files unless asked.
