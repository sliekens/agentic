...
Layer 1 makes a sentence readable. Layer 2 makes a reply usable. The two failures are different. A text can pass every word rule and still fail the reader, because the answer sat in paragraph four.
...
This copy of the skill holds the rules only. It ships no linter, so check the draft against the two lists below by hand.

Check before a final reply of more than 60 words, and before you write prose to a file. Make two passes, no more. Then send the reply one time.
